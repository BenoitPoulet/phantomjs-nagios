...
website_page_text = hash['log']['pages'][0]['pageText']
website_status = hash['log']['pages'][0]['status'].to_s
...
if website_status == "success"
    ...
    if options[:search] == true
        if !website_page_text.include? search_string
            output = output + ", String \"#{search_string}\" not found"
            exitcode = setExit(2, exitcode)
        else
            if options[:verbosity] >= 0
                output = output + ", String OK"
            end
        end
    end
